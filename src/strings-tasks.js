...
function removeFirstOccurrences(str, value) {
  const startIndex = str.includes(value) ? str.indexOf(value) : str.length;
...
function findLongestWord(sentence) {
  const arrayFromString = sentence.split(' ');
  let maxLengthWord = '';

  for (let i = 0; i < arrayFromString.length; i += 1) {
    if (arrayFromString[i].length > maxLengthWord.length) {
      maxLengthWord = arrayFromString[i];
    }
  }
...
function extractNameFromTemplate(value) {
  ...
  const lastElement = arrayFromString[arrayFromString.length - 1].slice(0, -1);

  return `${arrayFromString[1]} ${lastElement}`;
}
...
function getCardId(value) {
  ...
  return cards.indexOf(value) !== -1 && cards.indexOf(value);
}
